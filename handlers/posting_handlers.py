...
@dp.callback_query_handler(lambda c: c.data == 'return_to_start', state="*")
...
async def post_step_1(message: types.Message, state: FSMContext):
	...
	if message.from_user.id not in data:
		data['user_id'][message.from_user.id] = {"head":str(message.text),"body":None,"photo_id":None}
...
async def post_step_2(message: types.Message, state: FSMContext):
	...
	data = json_methods.read(DB_PATH)
	data['user_id'][str(message.from_user.id)]['body'] = str(message.text)
	json_methods.write(DB_PATH, data)
...
async def post_step_3(message: types.Message, state: FSMContext):
	...
	data = json_methods.read(DB_PATH)
	data['user_id'][str(message.from_user.id)]['photo_id'] = message.photo[0].file_id
	json_methods.write(DB_PATH, data)
...
async def review_new_post(callback_query: types.CallbackQuery):
	...
	await bot.send_photo(callback_query.from_user.id, data['user_id'][str(callback_query.from_user.id)]['photo_id'], caption = message_text, reply_markup = keyboards.agreement_to_send_post_kb)
...
async def send_new_post(callback_query: types.CallbackQuery):
	...
	await bot.send_message(callback_query.from_user.id, message_text, reply_markup = keyboards.creation_agreement_kb, parse_mode = ParseMode.MARKDOWN)
